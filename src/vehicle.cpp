...
Vehicle::Vehicle(double s, double d, double v, double a, double yaw, string state) {
...
vector<Vehicle> Vehicle::choose_next_state(map<int, vector<Vehicle>> predictions,
                                           std::function<double(Vehicle, map<int, vector<Vehicle>>, vector<Vehicle>)> calculate_cost) {
  /*
  ...
  */
  ...
  if (get_vehicle_ahead(predictions, lane, vehicle_ahead)) {
    ...
    if (get_vehicle_behind(predictions, lane, vehicle_behind)) {
      ...
      auto max_velocity_in_front = (double) ((vehicle_ahead.s - this->s - this->preferred_buffer) + vehicle_ahead.v - 0.5 * (this->a));
...
double Vehicle::distance_from_me(Vehicle &other, double time_delta) {
...
double Vehicle::accelerate(double factor) {
...
double Vehicle::decelerate(double factor) {
  double new_a = this->a - factor;
  if (abs(new_a) <= this->max_acceleration) {
    this->a = new_a;
  }
